...
updates_dir=/data/palyrimos_updates

if [ ! -f "$1" ]; then
   echo "Usage: $0 ZIP [UNVERIFIED]"
   echo "Push ZIP to $updates_dir and add it to Updater"
   echo
   echo "The name of ZIP is assumed to have palyrimos-VERSION-DATE-TYPE-* as format"
...
# Assume palyrimos-VERSION-DATE-TYPE-*.zip
...
adb shell "killall org.palyrim.updater 2>/dev/null"
adb shell "sqlite3 /data/data/org.palyrim.updater/databases/updates.db" \
